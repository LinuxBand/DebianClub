...
Debian プロジェクトリーダー（DPL）の選挙が進行中です：
...
- 安定版（Debian 13）と oldstable（Debian 12）はともに積極的なセキュリティメンテナンス下にあります
- LTS（Debian 11）も引き続き DLA 通知を公開しています
